...
function SimpleShader.init()
    ...
    local function newShaderFunction(name, pixelCode, vertexCode)
        if not vertexCode then
            if not pixelCode then
                pixelCode = name;
                name = nil;
            else
                if love.filesystem.getInfo(name) or string.len(name) > 25 then -- assuming that if its a valid file or is over 25 chars then its a shader
                    vertexCode = pixelCode;
                    pixelCode = name;
                    name = nil;
                end
            end
        ...
        if name == nil then
            ...
            while self.allShaders[name] do
                assert(curIter < maxIter, "couldnt locate a valid new shader name in a reasonable time");
                curIter = curIter + 1;
                
                if char < 26 then
                    char = char + 1;
                    name = string.sub(name, 0, string.len(name) - 1) .. string.char(97 + char);
                else
                    char = 0;
                    name = name .. string.char(97);
                end
            end
        end
